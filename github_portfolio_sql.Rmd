...
{% set specific_cgs = eval_sql("SELECT cons_group_table from cons_groups where lower(name) LIKE '%example - %'") %}
...
{%endfor%};    
UPDATE {{tmp("results")}} 
SET {%for l in specific_cgs%} `{{get_group_name(l.cons_group_table) | slice(0,63) | trim}}` = 0{% if not loop.last %},{% endif %} 
{%endfor%};
